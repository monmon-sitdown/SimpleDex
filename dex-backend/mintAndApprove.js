...
const tTokenAddress = "0x87a0ee49199346B2fcdF54FbCc9C1150d95a22fF";

async function main() {
  // 获取 Token0 和 Token1 的合约实例
  const tToken = new ethers.Contract(tTokenAddress, tokenABI, wallet);
  ...
  // 铸造代币
  const mintTx0 = await tToken.mint(recipientAddress, amountToMint);
  await mintTx0.wait();
  ...
  // Approve the contract to use the tokens
  const dexAddress = "0x090Bc3ff8116D285ad616aF3A9dF066B64b3126F"; // DEX contract address
  const approveTx0 = await tToken.approve(dexAddress, amountToMint);
